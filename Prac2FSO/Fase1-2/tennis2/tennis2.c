...
int tecla, contador = -1; // tecla pulsada y estado de la partida
...
void * mou_paleta_usuari(void * cap) {
	do {
			...
			if (moviments == -1)  // Si está desactivado los movimientos, continuamos
				...
				if (moviments > 0) {
					if ((tecla == TEC_AMUNT) || (tecla == TEC_AVALL))
						moviments--;
					pthread_mutex_unlock(&mutex_global);
...
int main(int n_args, const char *ll_args[]) {
	...
	do { /********** bucle principal del joc **********/
		...
		if (moviments == -1) {
			...
			sprintf(buffer, "MovFets: %d MovRestants: %d Temps: %2d:%2d", (movMax - moviments), moviments, min, seg);
